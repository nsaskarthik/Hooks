...
from hooks.context import session_key  # noqa: E402
        ...
        if session_id and config.state_dir.exists():
            ...
            if target.is_file():
                try:
                    ...
                    pass
        # The documented SessionEnd field is "end_reason"; keep "reason" as a
        # fallback for older/alternate payloads.
        end_reason = input_data.get("end_reason", input_data.get("reason", ""))
        audit_event("SessionEnd", input_data,
                    extra={"reason": end_reason,
