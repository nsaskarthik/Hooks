...
def _usage_int(usage: Any, key: str) -> int:
    """Read an integer token count from a usage dict OR object, never raising.

    Token usage may arrive as a dict (API backend), an SDK object, or be missing;
    coercing here keeps any oddity from escaping the :class:`LLMError` contract.
    """
    if usage is None:
        return 0
...
def _complete_api(model, system, user, *, max_tokens, timeout, api_key):
    ...
    try:
        ...
        return {
            "text": text,
            "input_tokens": _usage_int(getattr(message, "usage", None), "input_tokens"),
            "output_tokens": _usage_int(getattr(message, "usage", None), "output_tokens"),
            "model": model,
        }
    except LLMError:
        raise
