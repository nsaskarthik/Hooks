...
LE = Path(__file__).resolve().parents[1]
...
def test_session_end_clears_only_current_session_state(tmp_path):
    from hooks.context import session_key
    ...
    mine.write_text("{}")
    other.write_text("{}")
    proc = _run("session_end.py", {"session_id": "s1", "end_reason": "clear"}, tmp_path)
    assert proc.returncode == 0
    ...
    assert rec["reason"] == "clear"   # end_reason from the documented payload is captured
