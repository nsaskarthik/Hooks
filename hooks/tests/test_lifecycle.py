...
POST = _ROOT / "post_tool_use.py"
SUBMIT = _ROOT / "user_prompt_submit.py"


# --- subprocess helpers ---

# Hook flags / auth that must be cleared so inherited env can't change behavior.
_HOOK_ENV = ("ANTHROPIC_API_KEY", "CLAUDE_CODE_OAUTH_TOKEN", "ENABLE_TDD",
             "ENABLE_PROMPT_REWRITE", "REFINE_CONFIDENCE_THRESHOLD")
...
def _run_script(script: Path, payload: dict, log_dir: Path, env_extra: dict | None = None):
    ...
    for key in _HOOK_ENV:
        env.pop(key, None)
    env["LLM_BACKEND"] = "api"   # deterministic: no key -> llm unavailable -> skip
...
def test_submit_skips_without_api_key(tmp_path):
    proc = _run_script(SUBMIT, {"prompt": "fix teh bug", "session_id": "s1"}, tmp_path)
    assert proc.returncode == 0 and proc.stdout.strip() == ""
...
def test_submit_injects_corrected_context(tmp_path, monkeypatch, mock_llm):
    ...
    out = _run_main(user_prompt_submit.main,
                    {"prompt": "fix the athentication bug in the logn flow", "session_id": "s1"},
                    monkeypatch)
...
def test_submit_skips_slash_command(tmp_path, monkeypatch, mock_llm):
    ...
    from hooks import user_prompt_submit
    out = _run_main(user_prompt_submit.main, {"prompt": "/deploy staging", "session_id": "s1"}, monkeypatch)
...
def test_expansion_truly_rewrites_prompt(tmp_path, monkeypatch, mock_llm):
    ...
    from hooks import user_prompt_expansion
    ...
    payload = json.loads(out)["hookSpecificOutput"]
    # True rewrite uses hookSpecificOutput.expandedPrompt per the hooks docs.
    assert payload["hookEventName"] == "UserPromptExpansion"
...
def test_stop_tdd_block_then_pass(tmp_path, monkeypatch, mock_llm):
    ...
    _run_main(user_prompt_submit.main, {"prompt": "convert json to csv", "session_id": "sX"}, monkeypatch)
...
def test_subagent_stop_summarizes_transcript_not_agent_type(tmp_path, monkeypatch, mock_llm):
    # With the summary enabled, the model must be fed the subagent's real work
    # (its final transcript message), not just the agent_type string.
    _prime_config(monkeypatch, tmp_path)
    monkeypatch.setenv("SUBAGENT_SUMMARY", "true")
    transcript = tmp_path / "sub.jsonl"
    transcript.write_text(json.dumps(
        {"type": "assistant", "message": {"content": [
            {"type": "text", "text": "Refactored the auth module and added tests"}]}}) + "\n")

    captured = {}

    def responder(model, system, user):
        captured["user"] = user
        return "Refactored auth and added tests."

    mock_llm(responder)
    from hooks import subagent_stop
    _run_main(subagent_stop.main,
              {"agent_id": "a1", "agent_type": "Explore", "transcript_path": str(transcript)},
              monkeypatch)
    assert "Refactored the auth module" in captured["user"]   # real work, not "Explore"
    assert "Explore" not in captured["user"]


def test_stop_uses_assistant_message_field(tmp_path, monkeypatch, mock_llm):
    # The documented Stop payload carries assistant_message directly; validation
    # must work from it even when no transcript file is provided.
    cfg = _prime_config(monkeypatch, tmp_path, ENABLE_TDD="true", ENABLE_PROMPT_REWRITE="false")
    assert cfg.enable_tdd
    mock_llm({"tests": [{"name": "csv", "criterion": "output is CSV"}]})
    from hooks import user_prompt_submit, stop
    _run_main(user_prompt_submit.main, {"prompt": "convert json to csv", "session_id": "sM"}, monkeypatch)

    mock_llm({"results": [{"name": "csv", "passed": False, "reason": "not CSV"}]})
    out = _run_main(stop.main,
                    {"session_id": "sM", "assistant_message": "still JSON output",
                     "transcript_path": "", "stop_hook_active": False}, monkeypatch)
...
def test_submit_no_output_below_confidence(tmp_path, monkeypatch, mock_llm):
    ...
    from hooks import user_prompt_submit
    out = _run_main(user_prompt_submit.main, {"prompt": "orig", "session_id": "s1"}, monkeypatch)
...
def test_stop_respects_stop_hook_active(tmp_path, monkeypatch, mock_llm):
    ...
    from hooks import user_prompt_submit, stop
    _run_main(user_prompt_submit.main, {"prompt": "do x", "session_id": "sA"}, monkeypatch)
