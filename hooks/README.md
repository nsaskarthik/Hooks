# hooks — Prompt-rewrite + TDD validation hooks
...
- **MCP — Category 1 audit only.** Deterministic tools, so output is not validated.
  ...
  response_status, execution_duration_ms`. Note: the live Claude Code `PostToolUse`
  payload does not currently supply `model` or `duration_ms`, so `model_name` and
  `execution_duration_ms` are best-effort (empty/`0` when absent) — the keys are kept
  for spec/schema compatibility, not guaranteed values.

## Layout

All hook entry points and their helpers live flat at the `hooks/` root; only
`shared/` and `tests/` remain as subfolders.

```text
hooks/
├── user_prompt_submit.py, user_prompt_expansion.py, post_tool_use.py,
│   stop.py, pre_tool_use.py, … session_start.py …        (hook entry points)
├── tier_a_refine.py, tier_b_tdd.py, pipeline.py          (LLM tiers + shared pipeline)
├── mcp_audit.py                                          (deterministic MCP audit)
├── context.py                                            (prompt->Stop TDD state)
├── shared/             config.py, logger.py, errors.py, events.py,
│                       models_registry.py, llm_client.py, notify.py, transcript.py
├── hooks.json          plugin hook registration
├── plugin.json         plugin manifest
├── settings.json · settings.example.json   project-local (non-plugin) registration
...
1. **Copy this `hooks/` directory into your project** (or keep it where it is).
...
3. **Register the hooks** by copying the `hooks` block from
   ...
   `${CLAUDE_PROJECT_DIR}/hooks/*.py`. A ready-to-copy file is
...
path to the scripts (replace `${CLAUDE_PROJECT_DIR}/hooks` with the absolute
install path).
...
```bash
pip install -r hooks/requirements-dev.txt   # pytest + runtime deps
python3 -m pytest hooks/tests -q            # run everything (35 tests)
python3 -m pytest hooks/tests -v            # verbose, per-test names
python3 -m pytest hooks/tests/test_lifecycle.py -q   # one file
```
...
echo '{"tool_name":"mcp__x__y","tool_input":{"q":"hi"},"tool_output":"ok","duration_ms":3}' \
  | python3 hooks/post_tool_use.py   # writes an MCP audit record
```
